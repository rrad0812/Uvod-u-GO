...
Zdravo, dobrodošli na kurs i hvala što ste učili Go. Nadam se da će vam ovaj
kurs pružiti sjajno iskustvo učenja.

Šta je Go?
==========

Go (takođe poznat kao Golang) je programski jezik razvijen u kompaniji Gugl
2007. godine, a otvorenog koda objavljen 2009. godine.

Fokusira se na jednostavnost, pouzdanost i efikasnost. Dizajniran je da
kombinuje efikasnost, brzinu i bezbednost statički tipiziranog i kompajliranog
jezika sa lakoćom programiranja dinamičkog jezika kako bi programiranje ponovo
bilo zabavno.

Na neki način, želeli su da kombinuju najbolje delove Pajtona i C++-a kako bi
mogli da izgrade pouzdane sisteme koji mogu da iskoriste prednosti višejezgarnih
procesora.

Zašto učiti Go?
===============

Pre nego što počnemo sa ovim kursom, hajde da razgovaramo o tome zašto bi
trebalo da naučimo Go.

1. Lako se uči
--------------
Go je prilično lak za učenje i ima podržavajuću i aktivnu zajednicu.

A pošto je višenamenski jezik, možete ga koristiti za stvari poput razvoja
bekenda, računarstva u oblaku i, u skorije vreme, nauke o podacima.

2. Brz i pouzdan
----------------
Što ga čini veoma pogodnim za distribuirane sisteme. Projekti kao što su
Kubernetes i Docker su napisani u Go-u.

3. Jednostavan, ali moćan
-------------------------
Go ima samo 25 ključnih reči što ga čini lakim za čitanje, pisanje i održavanje.
Sam jezik je koncizan.

Ali nemojte da vas jednostavnost zavara, Go ima nekoliko moćnih funkcija koje
ćemo kasnije naučiti na kursu.

4. Mogućnosti za karijeru
-------------------------
Go brzo raste i usvajaju ga kompanije svih veličina. A sa tim dolaze i nove
dobro plaćene mogućnosti za posao.

Nadam se da te je ovo zainteresovalo za Go. Hajde da počnemo sa ovim kursom.
...
MacOS
-----
Otvorite datoteku paketa koju ste preuzeli i pratite uputstva da biste
instalirali Go. Paket instalira Go distribuciju na /usr/local/go. Paket bi
trebalo da se smesti na /usr/local/go/bin direktorijum u vašu PATH promenljivu
okruženja. Možda ćete morati da ponovo pokrenete sve otvorene terminalne sesije
da bi promena stupila na snagu.

Proverite da li ste instalirali Go tako što ćete otvoriti komandnu liniju i
otkucati sledeću komandu:
...
Linux
-----
Uklonite sve prethodne instalacije Goa brisanjem /usr/local/gofascikle (ako
postoji), a zatim raspakujte arhivu koju ste upravo preuzeli u /usr/local,
kreirajući novo Go stablo u /usr/local/go:

$ rm -rf /usr/local/go && tar -C /usr/local -xzf go1.18.1.linux-amd64.tar.gz

Napomena: Možda ćete morati da pokrenete komandu kao root ili preko sudo-a.

Ne raspakujte arhivu u postojeće /usr/local/go stablo. Poznato je da ovo dovodi
do neispravnih Go instalacija.

Dodajte /usr/local/go/bin u promenljivu okruženja PATH. To možete učiniti
dodavanjem sledeće linije u vašu $HOME/.profile `or` /etc/profile (za
instalaciju na nivou celog sistema):

export PATH=$PATH:/usr/local/go/bin

Napomena: Promene napravljene u datoteci profila možda neće biti primenjene do
sledećeg puta kada se prijavite na računar. Da biste odmah primenili promene,
jednostavno pokrenite komande školjke direktno ili ih izvršite iz profila pomoću
komande kao što je
...
Podrazumevano, instalater će instalirati program "Idi u Program Files" ili
"Program Files" (x86). Lokaciju možete promeniti po potrebi. Nakon instalacije,
moraćete da zatvorite i ponovo otvorite sve otvorene komandne linije kako bi se
promene u okruženju koje je napravio instalater odrazile u komandnoj liniji.

Proverite da li ste instalirali Go.
	U operativnom sistemu Windows kliknite na meni Start.
	U polje za pretragu menija otkucajte cmd, a zatim pritisnite taster Enter.
	U prozoru komandne linije koji se pojavi, otkucajte sledeću komandu:
...
To je to što se tiče instalacije i podešavanja Go-a, hajde da započnemo kurs i
napišemo naš prvi "Hello, World!".

Hello world
===========

Hajde da napišemo naš prvi program "Hello world", možemo početi inicijalizacijom
modula. Za to možemo koristiti go mod komandu.

$ go mod init example

Ali čekajte... šta je modul? Ne brinite, o tome ćemo uskoro razgovarati! Ali za
sada, pretpostavimo da je modul u osnovi kolekcija Go paketa.

Krećemo se dalje, hajde sada da kreiramo main.go datoteku i napišemo program
koji jednostavno ispisuje "Hello world".
...
Ako se pitate, fmt je deo je standardne Go biblioteke, koja je skup osnovnih
paketa koje pruža jezik.
...
Zatim, imamo nešto uvoza.

import "fmt"

Na kraju, ali ne i najmanje važno, je naša main funkcija koja deluje kao ulazna
tačka za našu aplikaciju, baš kao i u drugim jezicima poput C, Java ili C#.

func main() {
	...
}

Zapamtite, cilj je ovde da vodite mentalnu belešku, a kasnije tokom kursa ćemo
detaljno učiti o funkcijama, i drugim stvarima!
...
4. Deklaracija sa pretpostavljenim tipom
Tip je izostavljen, ali će biti pretpostavljen:

var foo = "What's my type?"

5. Skraćena deklaracija
Skraćena deklaracija, ovde izostavljamo var ključnu reč, a tip je uvek implicitan.
Ovako ćemo videti promenljive koje se deklarišu većinu vremena. Takođe koristimo
:= deklaraciju plus dodelu.

foo := "Shorthand!"

Napomena: Skraćenice rade samo unutar "function" tela reči.

Konstante
---------
Takođe možemo deklarisati konstante pomoću const ključne reči. Koje, kao što
ime sugeriše, predstavljaju fiksne vrednosti koje se ne mogu ponovo dodeliti.
...
Odlično! Sada hajde da pogledamo neke osnovne tipove podataka dostupne u Gou.
Počevši od stringova.

String
------
U jeziku Go, string je niz bajtova. Deklarišu se ili pomoću dvostrukih navodnika
ili povratnih navodnika koji omogućuju da se string proteže preko više redova.
...
Bulova vrednost
---------------
Sledeći tip je bool koja se koristi za čuvanje bulovih vrednosti. Može imati dve
moguće vrednosti - true ili false.
...
----------------------------------
Go ima nekoliko ugrađenih tipova celih brojeva različitih veličina za čuvanje
označenih i neoznačenih celih brojeva.

Veličina generičkih tipova int i uint zavisi od platforme. To znači da je
širina 32 bita na 32-bitnom sistemu i 64 bita na 64-bitnom sistemu.
...
Ako ste primetili, postoji i uintptrtip neoznačenog celobrojnog pointera, koji
je celobrojna reprezentacija memorijske adrese. Ne preporučuje se njegova
upotreba, tako da ne moramo da brinemo o tome.

Pa koji bi trebalo da koristimo?

Preporučuje se da kad god nam je potrebna celobrojna vrednost, koristimo samo in
tosim ako nemamo konkretan razlog za korišćenje nekog od tipova celog broja ili
neoznačenog celog broja.

Bajt i runa
-----------
Golang ima dva dodatna celobrojna tipa koja se zovu byte i rune koji su alijasi
za tipove podataka uint8 i int32, respektivno.

type byte = uint8	// alias
type rune = int32   // alias

rune predstavlja Unikod kodnu tačku.
...
Pokretni zarez
--------------
Zatim, imamo tipove sa pokretnim zarezom koji se koriste za čuvanje brojeva sa
decimalnom komponentom.

Go ima dva tipa pokretnog zareza float32i float64. Oba tipa prate standard
IEEE-754.
...
Kompleksni brojevi
-----------------
U Gou postoje dva kompleksna tipa, complex128, gde su i realni i imaginarni
delovi float64 i complex64, gde su realni i imaginarni delovi float32.

Kompleksne brojeve možemo definisati ili koristeći ugrađenu funkciju complex
ili kao literale.
...
Nulte vrednosti
---------------
Sada hajde da razgovaramo o nultim vrednostima. Dakle, u programskom jeziku Go,
svakoj promenljivoj deklarisanoj bez eksplicitne početne vrednosti dodeljuje
se njena nulta vrednost. Na primer, deklarišimo neke promenljive:
...
Dakle, kao što vidimo, vrednosti int i float se dodeljuju kao 0, bool kao false
i stringkao prazan string. Ovo se sasvim razlikuje od načina na koji to rade
drugi jezici, većina jezika inicijalizuje nedodeljene promenljive kao null ili
undefined.

Ovo je odlično, ali šta su ti simboli procenta u našoj Printf funkciji? Kao što
ste već pretpostavili, oni se koriste za formatiranje i o njima ćemo saznati
kasnije.

Konverzija tipa
---------------
Sada kada smo videli kako tipovi podataka funkcionišu, hajde da vidimo kako se
vrši konverzija tipova.
...
I kao što vidimo, ispisuje tip kao float64i uint.

Imajte na umu da se ovo razlikuje od parsiranja.

Alias tipa
-----------
Alias tipa su uvedeni u od verzije Go 1.9. Oni omogućavaju programerima da
obezbede alternativno ime za postojeći tip i da ga koriste naizmenično sa
osnovnim tipom.
...
Definisani tipovi
-----------------
Na kraju, definisali smo tipove koji za razliku od alias tipova ne koriste znak
jednakosti.
...
Dakle, definisani tipovi rade više od pukog davanja imena tipu. Prvo definiše
novi imenovani tip sa osnovnim tipom. Međutim, ovaj definisani tip se razlikuje
od bilo kog drugog tipa, uključujući i njegov osnovni tip.

Stoga se ne može koristiti naizmenično sa osnovnim tipom kao alias tipa. U
početku je malo zbunjujuće, nadam se da će ovaj primer razjasniti stvari.
...
Kao što vidimo, ne možemo koristiti definisani tip naizmenično sa osnovnim
tipom, za razliku od alias tipa.

Formatiranje stringova
----------------------
U ovom tutorijalu ćemo naučiti o formatiranju stringova ili ponekad poznatom
kao šabloniranje.

fmt paket sadrži mnogo funkcija. Da bismo uštedeli vreme, razmotrićemo najčešće
korišćene funkcije. Počnimo sa fmt.Print našom glavnom funkcijom.
...
Kao što vidimo, fmt.Print formatira ništa, jednostavno uzima string i ispisuje
ga.

Zatim, imamo fmt.Println što je isto kao i fmt.Print, ali dodaje novi red na
kraju i takođe ubacuje razmak između argumenata.
...
Zatim, fmt.Printf poznat nam je i "Formatizator štampanja" , koji nam omogućava
formatiranje brojeva, stringova, bulovih vrednosti i još mnogo toga.
...
Kao što vidimo, to %sj e zamenjeno našom name promenljivom.

Ali pitanje je šta jeste %s i šta to znači?

Dakle, ovo su glagoli anotacije i oni govore funkciji kako da formatira
argumente. Pomoću njih možemo kontrolisati stvari poput širine, tipova i
preciznosti, a ima ih mnogo. Evo špargalice.

Sada, hajde da brzo pogledamo još nekoliko primera. Ovde ćemo pokušati da
izračunamo procenat i ispišemo ga u konzolu.
...
Recimo da želimo samo 77.78 preciznost od 2 poena, to možemo uraditi i
korišćenjem .2f.
...
Ovo nas dovodi do fmt.Sprint, fmt.Sprintln i fmt.Sprintf. One su u osnovi iste
kao i funkcije za štampanje, jedina razlika je što vraćaju string umesto da ga
ispisuju.
...
Dakle, kao što vidimo, Sprintf formatira naš ceo broj kao heksadecimalan ili
binarni i vraća ga kao string.
...
Odlično! Ali ovo je samo vrh ledenog brega... zato obavezno pogledajte
dokumentaciju za fmt paket.

Za one koji dolaze sa C/C++ pozadinom, ovo bi trebalo da deluje prirodno, ali
ako dolazite, recimo, sa Pajtona ili Javaskripta, ovo bi u početku moglo biti
malo čudno. Ali je veoma moćno i videćete da se ova funkcionalnost koristi
prilično intenzivno.
...
Switch
------
Zatim, imamo switchi zjavu, što je često kraći način za pisanje uslovne logike.

U programskom jeziku Go, komanda switch pokreće samo prvi slučaj čija je
vrednost jednaka uslovnom izrazu, a ne sve slučajeve koji slede. Stoga, za
razliku od drugih jezika, break izjava se automatski dodaje na kraj svakog
slučaja.

To znači da switch procenjuje slučajeve od vrha do dna, zaustavljajući se kada
je slučaj uspešan. Pogledajmo primer:
...
Takođe možemo koristiti fallthrough ključnu reč da prenesemo kontrolu na sledeći
slučaj čak i ako se trenutni slučaj možda nije podudarao.
...
I ako ovo pokrenemo, videćemo da nakon prvog podudaranja slučaja, naredba switch
nastavlja na sledeći slučaj zbog fallthrough ključne reči.

$ go run main.go
time to work!
let's party

Takođe ga možemo koristiti bez ikakvog uslova, što je isto kao i switch true.
...
Sada, usmerimo pažnju na petlje. Dakle, u Gou imamo samo jednu vrstu petlje, a
to je for petlja.

Ali je neverovatno svestran. Baš kao i if naredba, for petlja, ne zahteva nikakve zagrade ()za razliku od drugih jezika.
Za petlju

Počnimo sa osnovnim forciklusom.
...
Osnovna for petlja ima tri komponente odvojene tačka-zarezom:
    ...
    izraz uslova : koji se izračunava pre svake iteracije.
    post izjava : koja se izvršava na kraju svake iteracije.

Break i continue
----------------
Kao što se i očekivalo, Go takođe podržava i break i continue naredbe za
kontrolu petlje. Hajde da pokušamo sa brzim primerom:
...
Dakle, continue naredba se koristi kada želimo da preskočimo preostali deo petlje,
a break naredba se koristi kada želimo da izađemo iz petlje.

Takođe, init i post naredbe su opcione, tako da možemo da nateramo našu for
petlju da se ponaša kao while petlja.
...
Napomena: možemo ukloniti i dodatne tačke-zareze da bismo ga učinili malo
čistijim.

Večna petlja
------------
Konačno, ako izostavimo uslov petlje, ona se ponavlja zauvek, tako da se
beskonačna petlja može kompaktno izraziti. Ovo je takođe poznato kao petlja
zauvek.
...
U ovom tutorijalu ćemo razgovarati o tome kako radimo sa funkcijama u
programskom jeziku Go. Dakle, počnimo sa jednostavnom deklaracijom funkcije.

Jednostavna deklaracija
-----------------------
...
Hajde da mu prosledimo neke parametre.
...
Kao što vidimo, ispisuje našu poruku. Takođe možemo da napravimo skraćenu
deklaraciju ako uzastopni parametri imaju isti tip. Na primer:

func myNextFunction(p1, p2 string) {}

Vraćanje vrednosti
------------------
Sada hajde da vratimo i vrednost.
...
Višestruki povratak
-------------------
Zašto vraćati jednu vrednost istovremeno, kada možemo više? Go takođe podržava
višestruko vraćanje!
...
Imenovani povratak
------------------
Još jedna sjajna funkcija su named returns, gde se povratne vrednosti mogu
imenovati i tretirati kao sopstvene promenljive.
...
Obratite pažnju kako smo dodali return iskaz bez ikakvih argumenata, ovo je
takođe poznato kao naked returns.

Reći ću da, iako je ova funkcija zanimljiva, molim vas da je koristite pažljivo
jer bi to moglo smanjiti čitljivost većih funkcija.

Funkcije kao vrednosti
----------------------
Zatim, hajde da pričamo o funkcijama kao vrednostima, u Go-u su funkcije prve
klase i možemo ih koristiti kao vrednosti. Dakle, hajde da sredimo našu
funkciju i isprobamo je!
...
Zatvaranja
----------
Zašto bismo se tu zaustavili? Hajde da vratimo i funkciju i tako kreiramo nešto
što se zove zatvaranje. Jednostavna definicija može biti da je zatvaranje
vrednost funkcije koja referencira promenljive izvan svog tela.

Zatvaranja su leksički ograničena, što znači da funkcije mogu pristupiti
vrednostima u opsegu prilikom definisanja funkcije.
...
Kao što vidimo, dobijamo rezultat 15 jer sumje promenljiva povezana sa
funkcijom. Ovo je veoma moćan koncept i definitivno ga morate znati.

Varijadičke funkcije
--------------------
Sada pogledajmo varijadne funkcije, koje mogu prihvatiti nula ili više
argumenata koristeći elipsis ...operator.

Primer ovde bi bila funkcija koja može da doda gomilu vrednosti.
...
Baš kul, zar ne? Takođe, ne brinite o range ključnoj reči, o tome ćemo kasnije
razgovarati u kursu.

Zanimljivost : fmt.Println je varijadička funkcija, zato smo joj mogli
proslediti više vrednosti.

Init
----
U Gou, init je posebna funkcija životnog ciklusa koja se izvršava pre main
funkcije.

Slično kao main, init funkcija ne prima nikakve argumente niti vraća bilo kakvu
vrednost. Pogledajmo kako to funkcioniše na primeru.
...
Za razliku od main, može postojati više od jedne init funkcije u jednoj ili više
datoteka.

Za višestruke funkcije init u jednoj datoteci, njihova obrada se vrši redosledom
njihove deklaracije, dok se init funkcije deklarisane u više datoteka obrađuju
prema leksikografskom redosledu imena datoteke.
...
I ako ovo pokrenemo, videćemo da su init funkcije izvršene redosledom kojim su
deklarisane.
...
Funkcija init je opcionalna i posebno se koristi za bilo koje globalno
podešavanje koje može biti neophodno za naš program, kao što je uspostavljanje
veze sa bazom podataka, preuzimanje konfiguracionih datoteka, podešavanje
promenljivih okruženja itd.

Defer
-----
Na kraju, hajde da razgovaramo o defer ključnoj reči koja nam omogućava da
odložimo izvršavanje funkcije dok se okolna funkcija ne vrati.
...
Možemo li koristiti više odloženih funkcija? Apsolutno, ovo nas dovodi do onoga
što je poznato kao defer stek. Pogledajmo primer:
...
Dakle, Defer je neverovatno koristan i često se koristi za čišćenje ili
rukovanje greškama.

Funkcije se takođe mogu koristiti sa generičkim tipovima, ali ćemo o njima
razgovarati kasnije u kursu.
...
Šta su moduli?
--------------
Jednostavno definisano, modul je kolekcija Go paketa smeštenih u stablu datoteka
sa go.mod datotekom u korenu, pod uslovom da je direktorijum van $GOPATH/src.

Go moduli su predstavljeni u Go 1.11, koji donosi nativnu podršku za verzije i
module. Ranije nam je bila potrebna GO111MODULE=on zastavica da bismo uključili
funkcionalnost modula kada je bila eksperimentalna. Ali sada, nakon Go 1.13,
režim modula je podrazumevani za sav razvoj.

Ali čekajte, šta je GOPATH?

Pa, GOPATH je promenljiva koja definiše koren vašeg radnog prostora i sadrži
...
Važno je napomenuti da Go modul može odgovarati i Github repozitorijumu ako
planirate da objavite ovaj modul.

Sada, hajde da istražimo go.mod koja je datoteka koja definiše putanju modula,
a takođe i putanju uvoza koja se koristi za korenski direktorijum i njegove
zahteve zavisnosti.
...
A ako želimo da dodamo novu zavisnost, koristićemo go install komandu:

$ go install github.com/rs/zerolog

Kao što vidimo, kreirana je i go.sum datoteka. Ova datoteka sadrži očekivane
heševe sadržaja novih modula.

Možemo navesti sve zavisnosti koristeći go list komandu:

$ go list -m all

Ako se zavisnost ne koristi, možemo je jednostavno ukloniti pomoću go mod tidy
komande:
...
Prodaja je čin pravljenja sopstvene kopije paketa trećih strana koje vaš
projekat koristi. Te kopije se tradicionalno smeštaju unutar svakog projekta,
a zatim čuvaju u repozitorijumu projekata.
...
Šta su paketi?
--------------
Paket nije ništa drugo do direktorijum koji sadrži jednu ili više Go izvornih
datoteka ili drugih Go paketa.

To znači da svaka Go datoteka sa izvornim kodom mora pripadati paketu, a
deklaracija paketa se vrši na vrhu svake datoteke sa izvornim kodom na sledeći
način.

package <package_name>

Do sada smo sve uradili unutar package main. Po konvenciji, izvršni programi
(pod tim mislim na one sa main paketom) se nazivaju komande, drugi se
jednostavno nazivaju paketi.

Paket main bi takođe trebalo da sadrži main() funkciju koja je posebna funkcija
koja deluje kao ulazna tačka izvršnog programa.

Pogledajmo primer kreiranjem sopstvenog paketa custom i dodavanjem nekih
izvornih datoteka u njega kao što su code.go.

package custom

Pre nego što nastavimo dalje, trebalo bi da razgovaramo o uvozu i izvozu. Baš
kao i drugi jezici, i go ima koncept uvoza i izvoza, ali je veoma elegantan.

U osnovi, bilo koja vrednost (kao što je promenljiva ili funkcija) može se
izvesti i biti vidljiva iz drugih paketa ako je definisana velikim slovima.
...
u kojem su definisani. U našem slučaju, custompaket.
...
Obratite pažnju kako je ime paketa upravo prezime putanje uvoza.
...
Takođe možemo da koristimo aliase za naše uvoze kako bismo izbegli ovakve
kolizije.
...
Spoljne zavisnosti
------------------
U programu Go, nismo ograničeni samo na rad sa lokalnim paketima, već možemo
instalirati i eksterne pakete koristeći go install komandu kao što smo ranije
videli.

Dakle, hajde da preuzmemo jednostavan paket za evidentiranje
...
Takođe, obavezno proverite go doc paketa koje instalirate, koji se obično nalazi
u readme datoteci projekta. go doc analizira izvorni kod i generiše dokumentaciju
u HTML formatu. Referenca na njega se obično nalazi u readme datotekama.

Na kraju, dodaću da Go nema posebnu konvenciju "strukture foldera", uvek
pokušajte da organizujete svoje pakete na jednostavan i intuitivan način.

Radni prostori
==============

U ovom tutorijalu ćemo saznati o višemodulnim radnim prostorima koji su
predstavljeni u Go 1.18.

Radni prostori nam omogućavaju da radimo sa više modula istovremeno bez potrebe
za uređivanjem go.mod datoteka za svaki modul. Svaki modul unutar radnog
prostora se tretira kao korenski modul prilikom rešavanja zavisnosti.
...
Ovo je odlično, ali šta ako želimo da izmenimo stringutil modul od kog zavisi
naš kod?

Do sada smo to morali da radimo koristeći replace direktivu u go.mod datoteci,
ali sada hajde da vidimo kako možemo da koristimo radne prostore ovde.
...
Takođe ćemo dodati naš hellomodul u radni prostor.

$ go work use ./hello

Ovo bi trebalo da ažurira go.workdatoteku sa referencom na naš hellomodul.

go 1.18

use ./hello

Sada, hajde da preuzmemo i izmenimo stringutilpaket i ažuriramo Reverseimplementaciju funkcije.
...
Konačno, dodajmo examplepaket našem radnom prostoru.
...
Odlično, sada ako pokrenemo naš hellomodul primetićemo da Reverseje funkcija izmenjena.
...
Počevši od go fmt, koji formatira izvorni kod i taj jezik ga sprovodi tako da se možemo fokusirati na to kako bi naš kod trebalo da funkcioniše, a ne na to kako bi naš kod trebalo da izgleda.

$ go fmt

Ovo može delovati malo čudno u početku, posebno ako dolazite iz stručnjaka za Javaskript ili Pajton kao ja, ali iskreno, prilično je lepo ne brinuti o pravilima lintovanja.

Zatim, imamo go vetizveštaje o verovatnim greškama u našim paketima.

Dakle, ako napravim grešku u sintaksi, a zatim pokrenem go vet.

Trebalo bi da me obavesti o greškama.

$ go vet

Zatim, imamo go envkoja jednostavno ispisuje sve informacije o go okruženju, o nekim od ovih promenljivih za vreme izgradnje ćemo saznati kasnije.

Na kraju, imamo go dockoji prikazuje dokumentaciju za paket ili simbol, evo primera fmtpaketa.

$ go doc -src fmt Printf

Hajde da koristimo go helpkomandu da vidimo koje su druge komande dostupne.

$ go help

Kao što vidimo, imamo:

go fixpronalazi Go programe koji koriste stare API-je i prepisuje ih da koriste novije.

go generateobično se koristi za generisanje koda.

go installkompajlira i instalira pakete i zavisnosti.

go cleankoristi se za čišćenje datoteka koje generišu kompajleri.

Neke druge veoma važne komande su go buildi go test, ali ćemo o njima detaljnije saznati kasnije u kursu.
Izgradi
...
To možemo vrlo lako uraditi pomoću go buildkomande.
...
Ove promenljive okruženja nam pomažu da napravimo go programe za različite operativne sisteme i osnovne arhitekture procesora .

Možemo navesti sve podržane arhitekture pomoću go toolkomande.
...
U ovom tutorijalu ćemo razgovarati o pointerima. Šta su, dakle, pointeri?

Jednostavno definisano, pointer je promenljiva koja se koristi za čuvanje memorijske adrese druge promenljive.

показивачи

Može se koristiti ovako:

var x *T

Gde Tje tip kao što je int, string, float, i tako dalje.

Hajde da pokušamo jednostavan primer i vidimo ga u akciji.
...
Hmm, ovo štampa nil, ali šta je nil?

Dakle, nil je unapred deklarisani identifikator u Go-u koji predstavlja nultu vrednost za pointere, interfejse, kanale, mape i kriške.

Ovo je baš kao ono što smo naučili u odeljku o promenljivim i tipovima podataka, gde smo videli da "uninitialized" intima nultu vrednost 0, "a" boolima "false" i tako dalje.
...
Koristimo &operator ampersand da bismo referencirali memorijsku adresu promenljive.

$ go run main.go
0xc0000b8000

Ovo mora biti vrednost memorijske adrese promenljive a.
Dereferenciranje

Takođe možemo koristiti *operator zvezdice da bismo preuzeli vrednost sačuvanu u promenljivoj na koju pointer pokazuje. Ovo se naziva i dereferenciranje .

Na primer, možemo pristupiti vrednosti promenljive apreko pointera pkoristeći taj *operator zvezdice.
...
Ne samo da mu možemo pristupiti već ga i promeniti pomoću pointera.
...
Postoji i drugi način inicijalizacije pointera. Možemo koristiti newfunkciju koja uzima tip kao argument, dodeljuje dovoljno memorije da smesti vrednost tog tipa i vraća pointer na njega.
...
U ovom tutorijalu ćemo naučiti o strukturama.

Dakle, a structje korisnički definisan tip koji sadrži kolekciju imenovanih polja. U osnovi, koristi se za grupisanje povezanih podataka zajedno u jednu jedinicu.
...
Možemo definisati structovako:

type Person struct {}

Koristimo typeključnu reč da bismo uveli novi tip, nakon čega sledi ime, a zatim structključna reč da bismo naznačili da definišemo strukturu.

Sada, hajde da mu damo neka polja:
...
Sada kada imamo našu strukturu, možemo je deklarisati isto kao i druge tipove podataka.
...
Kao što vidimo, sva polja strukture su inicijalizovana svojim nultim vrednostima. Dakle, FirstNamei LastNamesu postavljeni na ""prazan string, a Ageje postavljeno na 0.

Takođe ga možemo inicijalizovati kao "struct literal" .
...
Obe izjave su jednake jer u Go-u ne moramo eksplicitno dereferencirati pointer. Takođe možemo koristiti ugrađenu newfunkciju.
...
Dakle, zipCodepolje neće biti izvezeno. Takođe, isto važi i za Personstrukturu, ako je preimenujemo u person, ona se takođe neće izvezti.
...
Kao što smo ranije pomenuli, Go ne podržava nužno nasleđivanje, ali možemo uraditi nešto slično sa ugrađivanjem.
...
Strukturna oznaka je samo oznaka koja nam omogućava da polju dodamo metapodatke koje se mogu koristiti za prilagođeno ponašanje pomoću reflectpaketa.
...
Strukture su vrednosni tipovi. Kada dodelimo jednu promenljivu drugoj, kreira se i dodeljuje se structnova kopija te promenljive .struct

Slično, kada prosledimo structdrugoj funkciji, funkcija dobija svoju kopiju struct.
...
Hajde da pričamo o metodama, ponekad poznatim i kao prijemnici funkcija.
...
Argument prijemnika ima ime i tip. Pojavljuje se između ključne reči ifunc imena metode.

Na primer, definišimo Carstrukturu.
...
Sada, hajde da definišemo metodu poput IsLatestove koja će nam reći da li je automobil proizveden u poslednjih 5 godina.

func (c Car) IsLatest() bool {
	return c.Year >= 2017
}

Kao što vidite, možemo pristupiti instanci Carkoristeći promenljivu prijemnika c. Volim da je smatram thisključnom reči iz objektno orijentisanog sveta.
...
Metode sa prijemnicima pointera

Svi primeri koje smo ranije videli imali su prijemnik vrednosti.

Kod prijemnika vrednosti, metoda radi na kopiji vrednosti koja joj je prosleđena. Stoga, sve izmene izvršene na prijemniku unutar metoda nisu vidljive pozivaocu.

Na primer, napravimo još jednu metodu pod nazivom UpdateNamekoja će ažurirati ime Car.
...
Izgleda da ime nije ažurirano, pa sada prebacimo naš prijemnik na tip pointera i pokušamo ponovo.
...
Kao što se i očekivalo, metode sa pointerkim prijemnicima mogu da menjaju vrednost na koju prijemnik pokazuje. Takve modifikacije su vidljive i pozivaocu metode.
Nekretnine

Hajde da vidimo i neka svojstva metoda!

    Go je dovoljno pametan da pravilno interpretira naš poziv funkcije i stoga su pozivi metoda prijemnika pointera samo sintaksički šećer koji Go pruža radi praktičnosti.

(&c).UpdateName(...)

    Možemo izostaviti i promenljivi deo prijemnika ako ga ne koristimo.
...
U ovom tutorijalu ćemo naučiti o nizovima i kriškama u Go-u.
Nizovi
Šta je niz?
...
Ovde nje dužina i Tmože biti bilo kog tipa kao što je ceo broj, string ili korisnički definisane strukture.
...
Takođe možemo inicijalizovati niz koristeći literale niza.
...
I slično kao i u drugim jezicima, elementima možemo pristupiti koristeći , indexjer su sačuvani sekvencijalno.
...
Sada, hajde da pričamo o iteraciji.

Dakle, postoji više načina za iteraciju kroz nizove.

Prvi je korišćenje petlje for sa lenfunkcijom koja nam daje dužinu niza.
...
Drugi način je korišćenje rangeključne reči sa forpetljom.
...
Takođe možemo dozvoliti kompajleru da zaključi dužinu niza koristeći ...elipse umesto dužine.
...
Dužina niza je deo njegovog tipa. Dakle, niz ai bsu potpuno različiti tipovi i ne možemo dodeliti jedan drugom.
...
Ovo nas dovodi do "Slice", pa šta je onda "slice"?

Isečak je segment niza. Isečci se nadovezuju na nizove i pružaju veću snagu, fleksibilnost i praktičnost.

кришка

Rezak se sastoji od tri stvari:

    pointer-referenca na osnovni niz.
    Dužina segmenta niza koji isečak sadrži.
    I kapacitet, što je maksimalna veličina do koje segment može da raste.

Baš kao i kod lenfunkcije, možemo odrediti kapacitet segmenta koristeći ugrađenu capfunkciju. Evo primera:
...
Da vidimo kako možemo deklarisati krišku.

var s []T

Kao što vidimo, ne moramo da navodimo nikakvu dužinu. Hajde da deklarišemo deo celih brojeva i vidimo kako to funkcioniše.
...
Dakle, za razliku od nizova, nulta vrednost kriške je nil.
Inicijalizacija

Postoji više načina za inicijalizaciju našeg segmenta. Jedan od načina je korišćenje ugrađene makefunkcije.
...
Slično nizovima, možemo koristiti literal kriške da inicijalizujemo naš krišku.
...
Na kraju, hajde da razgovaramo o nekim svojstvima kriški.

Kriške su referentni tipovi, za razliku od nizova.
...
Kriške se mogu koristiti i sa varijadnim tipovima.
...
Slično nizovima ili kriškama, možemo iterirati kroz mape pomoću rangeključne reči .
...
Dakle, kada koristiti generičke lekove? Možemo uzeti sledeće slučajeve upotrebe kao primer:
    ...
    Funkcije koje rade na nizovima, kriškama, mapama i kanalima.
...
Dakle, slično kao kod kriški, možemo inicijalizovati naš kanal koristeći ugrađenu makefunkciju.
